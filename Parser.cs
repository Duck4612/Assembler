        ...
        public static List<Statement> Parse(List<List<Token>> tokenLines)
        {
            ...
            for(int lineNumber = 0; lineNumber < tokenLines.Count; lineNumber++)
            {
                ...
                while (opCode == null && directive == null && lineIndex < tokenLine.Count)
                {
                    ...
                    {
                        case Token.Types.Label when label == null:
                        ...
                        case Token.Types.OPCode:
                            sourceTokens = [.. sourceTokens, tokenLine[lineIndex]];
                            opCode = tokenLine[lineIndex++].Text;
                            enteredCodeSection = true;
                            break;
                        case Token.Types.Directive when !enteredCodeSection:
                            sourceTokens = [.. sourceTokens, tokenLine[lineIndex]];
                ...
                if (opCode != null)
                {
                    ...
                    if (operands.Length != spec.Operands.Length)
                        throw new Exception($"Invalid operand set length for {opCode}: Expected {spec.Operands.Length} | Actual {operands.Length}");

                    for (int i = 0; i < operands.Length; i++) 
                    {
                        if (operands[i].Type != spec.Operands[i])
                            throw new Exception($"Invalid operand types for {opCode}: Expected {spec.Operands[i]} | Actual {operands[i].Type} ({operands[i].Text})");
                    }
                }
